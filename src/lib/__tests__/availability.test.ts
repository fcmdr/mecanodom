...
// Le tenant-scoped db est injecté (DI) : on passe un stub avec les seules
// méthodes lues par computeSlotsForDate. Pas de base, pas de contexte de requête.

const settingsMock = vi.fn();
const blockedMock = vi.fn();
const windowsMock = vi.fn();
const bookingMock = vi.fn();

const db = {
  bookingSettings: { findFirst: settingsMock },
  blockedDate: { findFirst: blockedMock },
  workingHours: { findMany: windowsMock },
  booking: { findMany: bookingMock },
...
beforeEach(() => {
  ...
  blockedMock.mockResolvedValue(null);
...
describe("computeSlotsForDate — fenêtre autorisée", () => {
  it("rejette une date passée", async () => {
    ...
    const res = await computeSlotsForDate(db, "2026-07-19", service, now);
    expect(res.slots).toEqual([]);
    expect(res.reason).toBe("Date passée");
  });

  it("accepte la borne exacte diff == maxAdvanceDays", async () => {
    windowsMock.mockResolvedValue([win("08:00", "12:00")]);
    const now = new Date("2026-07-01T08:00:00.000Z"); // Paris "2026-07-01"
    const res = await computeSlotsForDate(db, "2026-07-31", service, now); // diff = 30
  ...
  it("rejette diff == maxAdvanceDays + 1", async () => {
    ...
    const res = await computeSlotsForDate(db, "2026-08-01", service, now); // diff = 31
    expect(res.slots).toEqual([]);
...
describe("computeSlotsForDate — jours indisponibles", () => {
  it("renvoie « Jour indisponible » pour un jour bloqué", async () => {
    blockedMock.mockResolvedValue({ id: 1 });
    const now = new Date("2026-07-13T08:00:00.000Z");
    const res = await computeSlotsForDate(db, "2026-07-20", service, now);
    expect(res.slots).toEqual([]);
...
describe("computeSlotsForDate — génération & fuseau", () => {
  it("génère les créneaux d'un jour normal en été (DST +2)", async () => {
    windowsMock.mockResolvedValue([win("08:00", "12:00"), win("13:00", "18:00")]);
    const now = new Date("2026-07-13T08:00:00.000Z");
    const res = await computeSlotsForDate(db, "2026-07-20", service, now);
  ...
  it("respecte le fuseau en hiver (DST +1)", async () => {
    ...
    const res = await computeSlotsForDate(db, "2026-01-19", service, now);
...
describe("computeSlotsForDate — délai minimum", () => {
  it("coupe les créneaux avant now + minLeadHours (borne incluse gardée)", async () => {
    ...
    const res = await computeSlotsForDate(db, "2026-07-20", { durationMin: 30 }, now);
...
describe("computeSlotsForDate — durée & chevauchements", () => {
  it("exclut une durée qui ne rentre pas dans la fenêtre", async () => {
    windowsMock.mockResolvedValue([win("08:00", "09:00")]);
    const now = new Date("2026-07-13T08:00:00.000Z");
    const res = await computeSlotsForDate(db, "2026-07-20", { durationMin: 90 }, now);
    expect(res.slots).toEqual([]);
    expect(res.reason).toBe("Aucun créneau disponible");
  });

  it("exclut les chevauchements avec buffer", async () => {
    settingsMock.mockResolvedValue({ ...DEFAULT_SETTINGS, bufferMin: 15 });
    windowsMock.mockResolvedValue([win("08:00", "12:00")]);
...
describe("computeSlotsForDate — fallback settings", () => {
  it("applique les valeurs par défaut quand bookingSettings est null", async () => {
    ...
    windowsMock.mockResolvedValue([win("08:00", "09:30")]);
